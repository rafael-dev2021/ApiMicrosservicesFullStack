                        ...
                        ICategoryService categoryService) : Controller
{
    ...
    [HttpGet]
    [AllowAnonymous]
    public async Task<ActionResult<IEnumerable<ProductViewModel>>> Search(string keyword)
    {
        try
        {
            if (string.IsNullOrWhiteSpace(keyword))
            {
                return View("Index", await _productService.GetAllProducts(await GetAccessToken()));
            }
            else
            {
                return View("Index", await _productService.GetProductsByKeywordAsync(keyword, await GetAccessToken()));
            }
        }
